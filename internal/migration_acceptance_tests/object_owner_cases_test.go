...
var objectOwnerAcceptanceTestCases = []acceptanceTestCase{
	...
	{
		name:  "Change composite type owner",
		roles: []string{"owner_a", "owner_b"},
		oldSchemaDDL: []string{`
			CREATE TYPE owner_pair AS (a int, b text);
			ALTER TYPE owner_pair OWNER TO owner_a;
		`},
		newSchemaDDL: []string{`
			CREATE TYPE owner_pair AS (a int, b text);
			ALTER TYPE owner_pair OWNER TO owner_b;
		`},
		expectedHazardTypes: []diff.MigrationHazardType{diff.MigrationHazardTypeAuthzUpdate},
		expectedPlanDDL:     []string{`ALTER TYPE "public"."owner_pair" OWNER TO "owner_b"`},
	},
	{
