...
func (vsg *viewSQLGenerator) Alter(vd viewDiff) (partialSQLGraph, error) {
	// Mask Privileges (handled by the privilege generator below) and Description
	// (handled by an explicit COMMENT statement) so the structural-equality check
	// below only triggers ErrNotImplemented when something we cannot alter changed.
	oldMasked := vd.old
